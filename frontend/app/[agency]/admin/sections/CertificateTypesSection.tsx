...
export default function CertificateTypesSection({ onNavigate, action, itemId, onBack }: SectionProps) {
  ...
  const fetchTypes = async () => {
    try {
      const response = await certificateTypes.getAll('tenancy');
      setTypes(response.data.certificateTypes || []);
    } catch (error) {
      console.error('Error fetching tenancy document types:', error);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        ...
        await certificateTypes.create({ ...formData, display_name: formData.name, type: 'tenancy' });
      ...
      setMessage({
        type: 'error',
        text: error.response?.data?.error || 'Failed to save document type'
      });
    } finally {
      setSaving(false);
    }
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      setMessage({
        type: 'error',
        text: error.response?.data?.error || 'Failed to delete document type'
      });
    }
  };
  ...
  return (
    <div>
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Tenancy Document Types</h2>
          <p className="text-gray-600">Manage document types displayed on tenant tenancy pages</p>
        </div>
        <button
      ...
      <div className="mb-6 p-4 rounded-lg bg-blue-50 border border-blue-200">
        <div className="flex gap-3">
          ...
          <p className="text-sm text-blue-800">
            Once configured here, these document types will be available to upload when creating or managing a tenancy. Each tenancy stores its own set of documents, shared between all tenants on that tenancy.
          </p>
        </div>
      </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          ...
          <form onSubmit={handleSave} className="space-y-4">
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <label htmlFor="ten_has_expiry" className="text-sm text-gray-700">
      ...
      {types.length === 0 ? (
        <div className="bg-white rounded-lg shadow-md p-12 text-center">
          ...
          <p className="text-gray-600 mb-4">No tenancy document types defined yet</p>
          <button
