...
export default function CertificateTypesSectionBase({
  apiType,
  title,
  subtitle,
  infoText,
  ...
  iconTextClass,
  checkboxId,
}: CertificateTypesSectionBaseProps) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        ...
        await certificateTypes.create({ ...formData, display_name: formData.name, type: apiType });
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    <div className={`${iconColorClass} p-2 rounded-lg`}>
                      <svg className={`w-5 h-5 ${iconTextClass}`} fill="currentColor" viewBox="0 0 20 20">
