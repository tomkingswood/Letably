...
export default function ReportsSection({ onNavigate, action, itemId, onBack }: SectionProps) {
    ...
    try {
      switch (selectedReport) {
        ...
        case 'occupancy':
          ...
          break;
        case 'financial':
          const financialRes = await adminReports.getFinancialReport();
          // Financial report may have different structure
          setPortfolioData(financialRes.data);
          break;
        case 'arrears':
        ...
        case 'upcoming-endings':
          ...
          setUpcomingEndingsData(endingsRes.data);
          break;
      }
    } catch (err) {
  ...
  const reportTypes = [
    ...
    { id: 'financial', name: 'Financial Summary', description: 'Payment collection overview' },
          ...
          <div>
            ...
            {selectedReport === 'portfolio' && portfolioData && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <p className="text-2xl font-bold text-gray-900">{portfolioData.totalProperties || 0}</p>
                </div>
                <div className="bg-gray-50 rounded-lg p-4">
                  <p className="text-gray-600 text-sm">Total Bedrooms</p>
                  <p className="text-2xl font-bold text-gray-900">{portfolioData.totalBedrooms || 0}</p>
                </div>
                <div className="bg-gray-50 rounded-lg p-4">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                        {property.bedrooms.map((bedroom, idx) => (
                          <div key={idx} className="bg-gray-50 rounded p-3">
                            ...
                            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${getStatusColor(bedroom.status)}`}>
                              {bedroom.status}
                            </span>
                            {bedroom.tenant && <p className="text-xs text-gray-500 mt-1">{bedroom.tenant}</p>}
                          </div>
                        ))}
                      </div>
                    </div>
            ...
            {selectedReport === 'financial' && (
              <div className="text-center py-8 text-gray-600">
                ...
                <p className="text-sm mt-2">This shows payment collection and financial metrics.</p>
              </div>
            )}
            ...
            {selectedReport === 'arrears' && arrearsData && (
              <div>
                {arrearsData.totalArrears !== undefined && (
                  <div className="bg-red-50 rounded-lg p-4 mb-6">
                    <p className="text-red-600 text-sm">Total Outstanding Arrears</p>
                    <p className="text-3xl font-bold text-red-700">£{arrearsData.totalArrears.toFixed(2)}</p>
                  </div>
                )}
                {arrearsData.tenancies && arrearsData.tenancies.length > 0 ? (
                    ...
                    <table className="w-full">
                      <thead>
                        <tr className="border-b bg-gray-50">
                          ...
                          <th className="text-left py-3 px-4 font-semibold text-gray-700">Overdue Payments</th>
                          <th className="text-right py-3 px-4 font-semibold text-gray-700">Total Arrears</th>
                        </tr>
                      </thead>
                      <tbody>
                        {arrearsData.tenancies.map(t => (
                          <tr key={t.id} className="border-b hover:bg-gray-50">
                            <td className="py-3 px-4 font-medium">{t.tenant_name}</td>
                            <td className="py-3 px-4">{t.property_address}</td>
                            <td className="py-3 px-4">{t.overdue_payments}</td>
                            <td className="py-3 px-4 text-right font-medium text-red-600">£{t.total_arrears.toFixed(2)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                          ...
                          <tr key={t.id} className="border-b hover:bg-gray-50">
                            ...
                            <td className="py-3 px-4 text-right">
                              <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                                t.days_remaining <= 7 ? 'bg-red-100 text-red-800' :
                                t.days_remaining <= 14 ? 'bg-yellow-100 text-yellow-800' :
