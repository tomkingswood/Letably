...
export default function TenancyDetailPage({ params }: PageProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      router.push(`/${agencySlug}/admin?section=tenancies`);
    } catch (err: unknown) {
  ...
  const handleCreateRollingTenancy = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/admin/tenancies/${response.data.tenancy.id}`);
    } catch (err: unknown) {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href={`/${agencySlug}/admin?section=tenancies`} className="text-primary hover:underline">
            Back to Tenancies
          </Link>
        </div>
      </div>
        ...
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              {tenancy && (
                <p className="text-xl text-white/90">{tenancy.property_address}</p>
              )}
            </div>
            <div className="flex gap-3">
              <Link
                href={`/${agencySlug}/admin/maintenance?property_id=${tenancy?.property_id}`}
                className="bg-white/20 text-white hover:bg-white/30 px-4 py-2 rounded-lg font-medium transition-colors flex items-center gap-2"
              >
              ...
              <Link
                href={`/${agencySlug}/admin?section=tenancies`}
