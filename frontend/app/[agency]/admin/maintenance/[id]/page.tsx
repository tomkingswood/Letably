...
// Extended interface for admin page (includes property_id and has_landlord)
...
// Status/priority changes are converted to readable messages
...
export default function MaintenanceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleDeleteRequest = async () => {
    ...
    try {
      ...
      router.push(`/${agencySlug}/admin?section=maintenance`);
    } catch (err: unknown) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <Link href={`/${agencySlug}/admin?section=maintenance`} className="text-primary hover:underline">
            Back to Maintenance
          </Link>
        </div>
      </div>
        ...
        <div className="container mx-auto px-4 py-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div className="flex gap-3">
              <Link
                href={`/${agencySlug}/admin?section=maintenance`}
                className="px-4 py-2 text-gray-600 hover:text-gray-900 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors"
              >
                Back to List
              </Link>
              <Link
                href={`/${agencySlug}/admin/tenancies/${request.tenancy_id}`}
