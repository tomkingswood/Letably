  ...
  const fetchRequest = useCallback(async () => {
    try {
      ...
      const mappedRequest: MaintenanceRequest = {
        ...reqData,
        creator_name: [reqData.created_by_first_name, reqData.created_by_last_name].filter(Boolean).join(' ') || 'Unknown',
        creator_email: reqData.created_by_email,
        property_address: [reqData.address_line1, reqData.city].filter(Boolean).join(', ') || 'Unknown address',
        comments: comments || [],
