...
CREATE TABLE IF NOT EXISTS holding_deposits (
  ...
  reservation_days INTEGER CHECK (reservation_days >= 0),
  reservation_expires_at TIMESTAMPTZ,
  reservation_released BOOLEAN DEFAULT FALSE,
  status VARCHAR(30) NOT NULL DEFAULT 'awaiting_payment' CHECK (status IN ('awaiting_payment', 'held', 'applied_to_rent', 'applied_to_deposit', 'refunded', 'forfeited')),
